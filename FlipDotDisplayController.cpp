...
/*
  Returns a byte containing the panel colum address
  
  First 3 bits: inner column address (from 0 to 7)
  Next 2 bits: sub group of columns address (from 0 to 4)
  Last two bits: sub panels address (from 1 to 3)
*/
byte DotDisplay::generateColumnCode (byte col){  
 ...
 if(relcol == 25){
  subgroup=3;
	relcol = 7;//needs confirmation, might need to leave it at 0
 } else { 
     subgroup = ((relcol-1) / 8);
     relcol = (relcol-1) % 8;   
	//delay(10);
...
void DotDisplay::updateDisplayArray(char textMessage[][16], int pause){

	//for (int wordPos = 0; wordPos < sizeof(textMessage); wordPos++){
	
	int size = sizeof(textMessage)/sizeof(textMessage[0][0]);
	//Serial.println(size);
	//Serial.println((sizeof(textMessage)));
	
	
	for (int wordPos = 0; wordPos < 4; wordPos++){
		updateDisplay(textMessage[wordPos]);
		disablePins();
		delay(pause);
	}
